...
<title>My JSP 'AdUpdate.jsp' starting page</title>
...
<body>
	<form name="thisform" method="post">
		...
		<input type="button"value="Update" onclick="sel()">
	</form>
	...
	<%
		...
		String newadvisor = request.getParameter("advisor");
		...
		if (id != null) {
			...
			student.setAdvisor(Integer.parseInt(newadvisor));
			student.setConsumption(Double.parseDouble(consumption));
			studentDAO.update(student);
		}

			StudentDAO studentDAOtemp = DAOFactory.getStudentDAOInstance();
			List<Student> all = (List<Student>) studentDAOtemp
					.queryByAdvisor(advisor.getAdvisorId());
			Iterator<Student> iter = all.iterator();
			int i = 0;
			while (iter.hasNext()) {
				i++;
				Student studenttemp = iter.next();
				out.println("Studentname: " + studenttemp.getStudentname()
						+ "<br>");
				out.println("Password: " + studenttemp.getPassword()
						+ "<br>");
				out.println("Advisor: " + studenttemp.getAdvisor() + "<br>");
				out.println("ConsumptionSum: "
						+ studenttemp.getConsumption() + "<br>");
			}
